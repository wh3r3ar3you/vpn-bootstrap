...
2. создаёт временный `ipset` `blacklist_new`
...
6. заполняет `blacklist_new`
7. выполняет `ipset swap blacklist_new blacklist`
...
- убедитесь, что в репозитории нет реальных SSH ключей, токенов и приватных данных
- не коммитьте локальные логи и временные файлы
- проверьте README и название репозитория под ваш публичный namespace
- проверьте текст лицензии и при необходимости замените год/правообладателя
- убедитесь, что GitHub Actions включён для CI-проверок
