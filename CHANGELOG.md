...
- автоопределение драйвера основного сетевого интерфейса через sysfs с резервной проверкой через `ethtool -i`
- автоматическое распознавание `virtio_net` и проверка этого поведения в GitHub Actions
- обновление `actions/checkout` до актуальной версии 7

### Изменено

- `VPN_DISABLE_GRO` по умолчанию работает в режиме `auto`: GRO отключается только на `virtio_net`
- ручные значения `VPN_DISABLE_GRO=0` и `VPN_DISABLE_GRO=1` имеют приоритет над автоопределением
- базовый sysctl-файл переименован в `90-vpn-tuning.conf`, чтобы защитный профиль применялся после него
- старые `99-vpn-tuning.conf` и `99-vpn-rps.conf` при обновлении сохраняются как резервные копии и больше не перезаписывают активные значения
- базовые параметры sysctl вынесены в отдельный файл `config/90-vpn-tuning.conf`, чтобы bootstrap и ручное обновление нод использовали один источник
- `net.core.netdev_budget_usecs` установлен в `8000`, поскольку XanMod 6.18 не принимает значение `4000`
- добавлен автоматический тест порядка применения базового и защитного профилей sysctl
